...
#define Ymass			.6 // yukawa mass in GeV, now sigma
...
int main(int argc, char *argv[])
{
  ...
  snprintf(name,sizeof(name),"./string_A%1.4f_s%1.4f_g%2.2f.dat",A,sigma,g);
...
// the main updates start
  ...
  {  
    ...
    if(i_update % nmeas==0 && i_update >= NTHERM)
      {
          fprintf(fp, "%i %f\n", length, old_energy);			 // Write the string length
    ...
    printf("Energy increment at the step %4.i : %6.2f, string length: %4i, Full E: %4.2f\n", 
                                        i_update + 1, old_energy - update_en, length, old_energy);
