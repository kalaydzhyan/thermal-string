...
#define Ymass	    0.3 			 // yukawa mass in GeV, now sigma
#define massa       Ymass*a			 // mass times a, so distance will be in a units

float g = 1.0;					 // Gravitational constant
int   length = 0;				 // String length
int   breakpoint;				 // Number of point, where the first string ends
float temperature;
...
float pt0[dim] = {4.5, 4.5, 0.0};		 // Center of the temperature distribution
...
double gravit_energy(int array[][dim])
{
  ...
  for(p = 0; p < length; p++)
  {
    for(i = 1; i < length; i++)
    {
        dis = distance(array, p, (p+i) % length);
        energy += -g*exp(-massa*dis)*pow(TENSION*a, 2)/pow(a*dis, dim-2);
    }
  }
...
double gravit_energy_one_node(int array[][dim], int p1)
{
    ...
    for(i = 1; i < length; i++)
    {
        dis = distance(array, p1, (p1+i) % length);
        energy += -g*exp(-massa*dis)*pow(TENSION*a, 2)/pow(a*dis, dim-2);
    }
...
{
    double dis = distance(array, p1, p2);
    return -g*exp(-massa*dis)*pow(TENSION*a, 2)/pow(a*dis, dim-2);
}
...
bool update1(int array[][dim], int curpoint)
{
  ...
  if(curpoint == breakpoint) return false;      // Do not update ends of the strings
...
bool update2(int array[][dim], int curpoint)
{
  ...
  if(curpoint == breakpoint || curpoint == breakpoint-1) return false;      // Do not update ends of the strings
...
bool update3(int array[][dim], int curpoint)
{
  ...
  // Do not update ends of the strings
  if(curpoint == breakpoint || curpoint == breakpoint-1 || curpoint == breakpoint-2) return false;      
...
{
  FILE *init = fopen("./doublestring.ini", "r");
...
int main(int argc, char *argv[])
{
  ...
  old_energy = TENSION*a*(length - 2) + gravit_energy(string);
  ...
  snprintf(name,sizeof(name),"./dbstring_A%1.4f_s%1.4f_g%2.2f.dat",A,sigma,g);
...
// The main update cycle starts here
  ...
  {  
    ...
    { 
      ...
      if(update1(string, i)) 
      {
	 if(i < breakpoint) breakpoint += 2;	         // Update the number of the point, 
							 // where the first string ends
	 i+=2;
    ...
    for(i = 0; i < length - 3; i++)
    {
      ...
      if(update3(string, i) && length >= 4)		 // Don't let it be smaller than a plaquette
      {
	 if(i < breakpoint) breakpoint -= 2;		 // Update the number of the point, 
							 // where the first string ends
	 old_energy = new_energy;
   ...
   ////// Output begins /////
    if(i_update % nmeas==0 && i_update >= NTHERM)
      {
          fprintf(fp, "%i %f %i\n", length, old_energy, breakpoint);	 // Write the string length, energy and breaking point
    ...
    printf("Energy increment at the step %4.i : %6.2f, total length: %4i, total E: %3.4f\n",
                                             i_update + 1, old_energy - update_en, length-2, old_energy);
 ...
 // end of updates
  ...
  printf("Total energy (gravit. energy): %f  (%f) \n", TENSION*a*(length - 2) + gravit_energy(string), gravit_energy(string));
